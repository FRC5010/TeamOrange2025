...
public class ThriftyNovaSwerve extends SwerveMotor {
  ...
  /**
   * Configure the integrated encoder for the ThriftyNova swerve module. Sets the conversion factors
   * for position and velocity, and configures the CAN status frames.
   *
  ...
  public void setReference(double setpoint, double feedforward, double position) {
    if (RobotBase.isReal()) {
      if (isDriveMotor) {
        ...
        double convertedSetpoint =
            absoluteEncoder.map(it -> setpoint).orElse(setpoint / positionConversionFactor);
